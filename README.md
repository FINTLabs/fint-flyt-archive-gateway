...
| `GET`  | `/kodeverk/{type}`                                  | Returns cached kodeverk entries (administrativEnhet, arkivdel, dokumentstatus, journalposttype, etc.). | –            | `200 OK` array of `ResourceReference { "id": "<selfLink>", "displayName": "[code] Name #systemId" }`; arkivressurs also exposes `functionalId`, `name`, and `technicalId` when present. |
